...
**Decision:** Give `ProductStrip` a distinct state language. No backgrounds; instead a 2px gradient bar at the top edge of the strip that slides between items on hover and rests on the active item (light + dark, same colors). Items with `external: true` render a small up-right arrow after the label that pops in on hover/active (300ms `cubic-bezier(.16,1,.3,1)` with overshoot). The SVG slot is rendered for every item (transition-less and invisible on non-externals) so item widths and visual gaps stay uniform. Spacing: 48px between logo and first item, 18px between items. Indicator position is driven by `useLayoutEffect` + `ResizeObserver` on the nav, with inline `left`/`width` on a single absolutely-positioned `<span>`. `motion-reduce` suppresses both the slide and the arrow pop.
**Rationale:** A 2px top-edge indicator on a flat strip reads as "chrome that holds your place" the way browser tabs do, while pills read as "control you can press" — using different mechanics makes the role of each bar obvious without consumers needing to learn the convention. CSS-var-driven inline positioning avoids the React-tree re-renders that a state-based indicator would cause when hovering across items. Reserving the arrow slot on every item solves an optical-spacing bug: with the arrow only on externals, the text-to-text gap before each external item was ~16px narrower than the others.
**Alternatives considered:** Auto-detecting external links by comparing `href` origin to `window.location.origin` (rejected — couples behavior to runtime environment; same href flips between internal and external in dev vs prod). Centered short "tick" indicator instead of full-width (C3 in the brainstorm — rejected for being less assertive about location). Brand-dot prefix on the active item (B in the brainstorm — rejected, looked more "tool palette" than "shell chrome"). Sliding indicator that doesn't track hover (C2 — rejected, lost the playful "the chrome notices you" feel without giving anything back).

## Decision #82 — 2026-05-15

**Context:** Animating `NavItem` hover and active states. The end-state visual is "icon on the right of the label" while the rest state is "icon on the left." We needed a single mechanic that works in both the expanded sidebar (icon swaps sides relative to the label) and the collapsed icon rail (icon refreshes in place inside the bounding box).
**Decision:** Use a single `translateX` on a wider content band sitting behind an `overflow: hidden` mask. The band renders `[iconA] gap [label] gap [iconB]` and is 26px wider than the mask; default `transform: translateX(0)` keeps iconA + label visible, hover/active triggers `translateX(-26px)` so the band slides one icon-plus-gap to the left, revealing label + iconB. The same band carries two copies of the same icon. A scoped CSS block under `[data-collapsed="true"]` shrinks the mask to 16×16 and the band to 32px (gap 0), with the hover/active rule translating by -16px — same mechanic, smaller geometry. State is driven by `:hover` on the link OR `[data-active="true"]` on the link, so an active item never needs a separate animation path. Reduced-motion users get the state change with no transition.
**Rationale:** Transform-only animation avoids layout recalc on every frame and works identically in both modes — the only thing that changes between expanded and collapsed is the mask + band size, which is one CSS override scoped to the existing `data-collapsed` attribute. Sharing the hover and active end-state collapses what would otherwise be three states (rest, hover, active) into two transitions (rest ↔ end-state), which means an active item that is also hovered doesn't double-animate, and route changes between active items animate naturally as one band slides back to rest while another slides to end-state. The `data-slot` / `data-active` attribute hooks let all the animation logic live in `tokens.css` instead of being threaded through component class strings.
**Alternatives considered:** Animating slot widths and margins (rejected — triggers layout on every frame, and the asymmetry between expanded mode and collapsed mode forced mode-specific markup branches). Two absolutely-positioned icons that crossfade (rejected — fade-only doesn't carry the "icon moves" intent the design called for, and absolute positioning interacts poorly with the existing flex-based row). Animating in collapsed mode via a separate "icon-only" element nested inside `NavItem` (rejected — duplicates the icon prop into a second slot and makes consumers think about a state vocabulary the DS already handles).

## Decision #81 — 2026-05-15

**Context:** Refining nav item appearance in `ProductStrip` and `AppShellSidebar` `NavItem`. Two real problems with the existing treatment: (1) `hover:bg-muted` in light theme resolves to `primary-100` — exactly the active background, so hover and active were visually identical except for text weight; in dark theme `bg-muted` resolves to `base-900`, the surface color, so hover produced no visible change at all. (2) The dark-theme active state (`bg-primary-900` on `base-900`) read as muddy: two near-equal dark tones gave the active pill almost no edge against its surface.
**Decision:** Adopt a single primary-tinted state spectrum across both nav surfaces. Rest stays `text-muted-foreground`; hover and active share the same color story (light: `primary-100/50` → `primary-100`, both with `text-primary-700`; dark: `primary-500/8` → `primary-500/18`, both with `text-primary-200`), with active adding `font-medium` and intensity as the differentiator. Focus-visible adds a 2px primary outline (light: `primary-500`, dark: `primary-300`) at 2px offset, applied via `focus-visible:` so mouse interactions don't trigger it. Hover and active classes are mutually exclusive in the class list — an active item never picks up the lighter hover background (this matters because `hover:bg-X` has higher CSS specificity than `bg-Y` and would otherwise win).
**Rationale:** Hover and active feel like one design language, with intensity the only variable — easier to scan, easier to maintain. Dark-mode active no longer fights its surface: a translucent primary glow over the dark surface mirrors the soft-tint character of the light theme rather than switching to a separate "darker pill" vocabulary. Keyboard users get a clear, non-shifting focus ring without bothering mouse users. The mutual-exclusion branching is a cleanup of a real specificity bug in the original code.
**Alternatives considered:** Solid `primary-500` pill for active (rejected as too aggressive). Indicator-only treatment with no fill (rejected — too quiet for primary navigation). Inverting the light/dark relationship with a saturated `primary-700` pill in dark mode (rejected — broke the soft-tint character that ties the two themes together). Applying the new hover treatment to section title rows and the collapse toggle too (rejected — they are sidebar chrome, not nav items, and conflating the two would dilute the signal that primary tint = navigation).
