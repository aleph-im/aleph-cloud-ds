...
- ProductStrip component (`@aleph-front/ds/product-strip`) for cross-app navigation across the Aleph product family: declarative `apps` list of `{ id, label, href }`, `activeId` to mark the current app (`aria-current="page"`), `logoHref` for the logomark anchor, optional `right` slot. Active tab reuses the existing `bg-primary-100`/`dark:bg-primary-900` pill convention. First of the shell primitives — preview lives in the new `Application Shell` sidebar group.
- AppShellSidebar component family (`@aleph-front/ds/app-shell-sidebar`): `AppShellSidebar` shell with expanded ↔ icon-rail collapse and a built-in collapse toggle anchored at the bottom, plus an optional `footer` prop rendered to the left of the toggle when expanded (hidden when collapsed; toggle returns to centered) — typical use is a version string; `AccordionSection` with per-section open/closed state; `NavItem` with icon + label, `active` (announces `aria-current="page"`), and an `asChild` prop (cloneElement, mirroring `Button.asChild`) that lets consumers plug in routing-aware links (Next.js `Link`, React Router `Link`, etc.) — `NavItem` also accepts the full `AnchorHTMLAttributes` shape so `onMouseEnter`/`onFocus` hover-prefetch handlers pass through to the rendered anchor (bare or cloned). Single-tree rail-hide pattern: section titles and item labels carry a `rail-hide` class and a global `[data-collapsed="true"] .rail-hide { display: none }` rule (in `tokens.css`) handles the visual swap. Backed by two SSR-safe hooks — `useSidebarCollapse` (`@aleph-front/ds/use-sidebar-collapse`) and `useAccordionState` (`@aleph-front/ds/use-accordion-state`) — both return `boolean | null` (null while hydrating) with localStorage persistence under `sidebar.collapsed` and `sidebar.section.<sectionId>`. Inner nav is `min-h-0 flex-1 overflow-y-auto` so the toggle row stays anchored when content overflows.
