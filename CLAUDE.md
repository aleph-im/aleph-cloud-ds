...
- AppShellSidebar component family (`@aleph-front/ds/app-shell-sidebar`): `AppShellSidebar` shell with expanded ↔ icon-rail collapse and a built-in collapse toggle anchored at the bottom; `AccordionSection` with per-section open/closed state; `NavItem` with icon + label, `active` (announces `aria-current="page"`), optional `onClick`. Single-tree rail-hide pattern: section titles and item labels carry a `rail-hide` class and a global `[data-collapsed="true"] .rail-hide { display: none }` rule (in `tokens.css`) handles the visual swap. Backed by two SSR-safe hooks — `useSidebarCollapse` (`@aleph-front/ds/use-sidebar-collapse`) and `useAccordionState` (`@aleph-front/ds/use-accordion-state`) — both return `boolean | null` (null while hydrating) with localStorage persistence under `sidebar.collapsed` and `sidebar.section.<sectionId>`. Inner nav is `min-h-0 flex-1 overflow-y-auto` so the toggle row stays anchored when content overflows.
