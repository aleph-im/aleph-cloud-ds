...
- Tabs component (Radix UI) with composable API (Tabs, TabsList, TabsTrigger, TabsContent), sliding active indicator via MutationObserver, text nudge animation, arbitrary trigger children (badges, subscripts), disabled state, keyboard navigation, pill variant (`variant="pill"`) with gradient-fill sliding indicator for segmented-control style
